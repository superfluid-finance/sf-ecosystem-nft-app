...
export const SwitchNetwork = ({ idealNetwork, type }: { idealNetwork?: number, type: "mint" | "claim stream" }) => {
    ...
    const performSwitchChainToIdealNetwork = useCallback(async (wallet: any, idealNetwork: number) => {
      try {
    ...
    return (
      <div className="flex flex-col gap-y-2">
        <button className="cursor-pointer w-full bg-sf-green active:border-transparent active:outline-none focus:border-transparent focus:outline-none hover:border-transparent hover:outline-none rounded-[0.625rem] font-medium text-white" onClick={handleSwitchNetwork}>
        { loading ? 
        <LoadingSpinner /> :
