...
export const StreamInfo = ({ streamInfo }: { streamInfo: StreamInfoType }) => {
    return (
      <div className="flex flex-col justify-center items-center">
        <span className="text-xs font-medium text-[#121413]/87">Total amount streamed</span>
        ...
        <div className="font-medium text-[2rem] text-[24px] bigscreen:text-lg leading-[1] flex gap-x-2 items-end">
          <p><FlowingBalance balance={streamInfo?.balance} balanceTimestamp={streamInfo?.balanceTimestamp} flowRate={streamInfo?.flowRate} /></p>
          <span className="text-base bigscreen:text-md text-sf-green -ml-1">USDCx</span>
        </div>
        </div>
      </div>
    )
  ...
  return (
    ...
    <div className="w-screen h-screen fixed top-0 left-0 z-[100] flex items-center justify-center">
     ...
     <div className="p-12 bg-white relative max-w-[36rem] rounded-[1.25rem]">
      <img className="absolute top-6 right-6 w-6 h-6 cursor-pointer" src={XIcon} onClick={() => {
        setUpdate(true)
        setModalOpen(false)
      }}/>
      
      <div className="flex flex-col gap-y-8 !bg-cover !bg-no-repeat !bg-[center_1rem]" style={{ background: `url(/modal-sf-bg.png)`}}>
        <div>
          <h4 className="text-md font-bold text-[#2E3A47] text-center">Your stream is running</h4>
          <p className="text-center text-darkgrey mt-[0.2rem]">Lorem ipsum dolor sit amet consectetur. Auctor orci est et bibendum. Metus quis sed tortor tincidunt tellus.</p>
        </div>
