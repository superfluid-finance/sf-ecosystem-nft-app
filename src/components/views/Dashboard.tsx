...
export const MintStatusContext = createContext<{ update: boolean, setUpdate: Function }>(null!)

/** Preview image of the NFT */
const NFTPreview = () => {
  return (
    <div className="w-full md:w-1/2 rounded-2xl">
      <img src={PreviewImage} className="w-full h-full rounded-2xl object-cover" />
    </div>
  )
}
...
const DropdownSelect = () => {
  ...
  const Option = ({ chainInfo } : { chainInfo: NFTChain }) => {
    ...
    let countdown =  useCountdown((chainMintInfos[chainInfo.viemChain.id]?.flowRunsUntil || 0)*1000)
      ...
      <div className="cursor-pointer option flex gap-x-3 border-b border-[#EAEFF4] py-5">
        ...
        <div className="flex-col w-full">
          ...
          <div className="w-full flex justify-between items-center">
            <p className="font-medium text-sm text-darkgray">Stream will be flowing for:</p>
            { chainMintInfos[chainInfo.viemChain.id]?.flowRunsUntil? <Timer countdown={countdown} /> : <p className="flex gap-x-2 text-sm text-darkgray">N/A</p> }
          </div>

          <div className="w-full flex justify-between items-center">
            <p className="font-medium text-sm text-darkgray">Minted</p>
            <p className="text-sm text-darkgray">{chainMintInfos[chainInfo.viemChain.id]?.mintedCount || 0}</p>
          </div>
  ...
  const SelectedView = () => (
    ...
    <div className={`rotate-180 transition-all ${isActive ? '-rotate-0' : ''}`}>
     <DropdownArrow />
    </div>
  </div>
  )
  ...
  return (
    <div className="dropdown relative rounded-[0.625rem]">
      <SelectedView />
      <div className={`option-list max-h-[40vh] overflow-y-scroll bg-white dropdown-boxshadow mt-2 absolute w-full px-5 py-1 rounded-[0.625rem] ${ isActive ? 'block' : 'hidden'}`}>
        { NETWORK_LIST
          .filter((n: NFTChain) => n.name != selected.name)
          .map((network: NFTChain) => 
          <div className="option-wrapper" onClick={() => {
            setSelected(network)
            setIsActive(false)
          }}>
...
const MintInfoBox = () => {
  ...
  let countdown = useCountdown((chainMintInfos[selected.viemChain.id]?.flowRunsUntil || 0)*1000)
  let lastMintedTimestamp; 

  if (chainMintInfos[selected.viemChain.id] && chainMintInfos[selected.viemChain.id].flowRunsUntil) {
    // @ts-ignore
    lastMintedTimestamp = (chainMintInfos[selected.viemChain.id]?.lastMintedTimestamp || 0)*1000
  }
  ...
  return (
    <div className="dropdown-boxshadow p-5 rounded-[0.625rem] font-medium w-full flex flex-col gap-y-2.5">
      <div className="w-full text-sm flex justify-between items-center">
        ...
        <p className="text-black-2">{chainMintInfos[selected.viemChain.id]?.mintedCount ?? 0}</p>
      </div>
      <div className="w-full text-sm flex justify-between items-center">
        <p className="text-darkgray">Last Mint:</p>
        <p className="text-black-2">{ lastMintedTimestamp ? new Date(lastMintedTimestamp).toDateString() : 'N/A' }</p>
      </div>
      <div className="w-full text-sm flex justify-between items-center">
        <p className="text-darkgray">Stream will be flowing for:</p>
        <div className="flex gap-x-2 items-center">
          { lastMintedTimestamp ? <Timer countdown={countdown} className="!text-black-2" /> : <p className="flex gap-x-2 text-sm text-black-2">N/A</p> }
      ...
      <div className="w-full text-sm flex justify-between items-center">
        ...
        <p className="text-black-2">{selected.price} {selected.ticker}</p>
      </div>
    </div>
  )
}
...
const UserMintInfoBox = ({ includesDetailedInfo = false } : { includesDetailedInfo?: boolean} ) => {

  const { mintedChain, timestamp, tokenId } = useContext(UserMintInfoContext)
  const { chainMintInfos } = useContext(ChainMintInfosContext)

  let countdown = useCountdown((chainMintInfos[mintedChain?.viemChain?.id]?.flowRunsUntil || 0)*1000)
  let lastMintedTimestamp; 

  if (mintedChain && chainMintInfos[mintedChain.viemChain.id] && chainMintInfos[mintedChain.viemChain.id].flowRunsUntil) {
    // @ts-ignore
    lastMintedTimestamp = (chainMintInfos[mintedChain.viemChain.id]?.lastMintedTimestamp || 0)
  }
  ...
  return (
    <div className="dropdown-boxshadow p-5 rounded-[0.625rem] font-medium w-full flex flex-col gap-y-2.5">
      ...
      <div className="w-full text-sm flex justify-between items-center">
        ...
        <p className="text-black-2">{ new Date(timestamp*1000).toDateString() }</p>
      </div> :
      (<>
      <div className="w-full text-sm flex justify-between items-center">
      ...
      <div className="w-full text-sm flex justify-between items-center">
        <p className="text-darkgray">NFTs Minted:</p>
        <p className="text-black-2">{ chainMintInfos[mintedChain?.viemChain?.id]?.mintedCount || 0 }</p>
      </div>
      <div className="w-full text-sm flex justify-between items-center">
        <p className="text-darkgray">Last Mint:</p>
        <p className="text-black-2">{ lastMintedTimestamp ? new Date(lastMintedTimestamp*1000).toDateString() : 'N/A' }</p>
      </div>
      <div className="w-full flex justify-between items-center">
        <p className="font-medium text-sm text-darkgray">Stream will be flowing for:</p>
        <div className="flex gap-x-2 items-center">
          { lastMintedTimestamp ? <Timer countdown={countdown} /> : <span className="flex gap-x-2 text-sm text-darkgray">N/A</span> }
          <div className="tooltip cursor-pointer">
            <img src={InfoIcon} />
            <span className="tooltiptext p-2 bg-white">{TOOLTIP_TEXT}</span>
          </div>
...
const ToMintContent = () => {
  return (
    <>
    <h1 className="text-md font-bold mb-2 text-[#2E3A47]">Superfluid Ecosystem NFT</h1>
      <h2 className="text-sm text-darkgray mb-8">Mint a Superfluid Ecosystem NFT to be a part of the official launch of the General Distribution Agreement!</h2>
...
const ClaimStreamContent = () => {
  ...
  const [ requireSwitchNetwork, setRequireSwitchNetwork ] = useState<boolean>(false)
  ...
  return (
  <>
  <h1 className="text-md font-bold mb-2 text-[#2E3A47]">Claim your Stream!</h1>
  <h2 className="text-sm text-darkgray mb-8">Lorem ipsum dolor sit amet consectetur. Pharetra ante eget lacus feugiat orci lorem. Non quam pretium purus aliquet turpis velit.</h2>

  <div className="flex flex-col gap-y-5">
    <UserMintInfoBox />
    { requireSwitchNetwork ? <SwitchNetwork idealNetwork={mintedChain?.viemChain?.id} type="claim stream" /> : <ClaimStreamButton /> }
  </div>
  </>
  )
}
...
const MintedAndClaimedContent = () => {
  ...
  return (
  <>
  <h1 className="text-md font-bold mb-2 text-[#2E3A47]">Lorem ipsum dolor sit amet</h1>
  <h2 className="text-sm text-darkgray mb-8">Lorem ipsum dolor sit amet consectetur. Faucibus tellus vitae at duis dui tellus in. Morbi ac tempus quis dolor non ut nulla feugiat.</h2>
  ...
  <div className="flex flex-col gap-y-5">
    ...
    <div className="flex flex-col gap-y-2">
      ...
      <p className="text-xs text-darkgray">If you want to mint another NFT connect different wallet.</p>
    </div>
  </div>
  </>
  )
    ...
    <MintStatusContext.Provider value={mintContextProviderProps}>
      <div className="w-full md:w-1/2 p-8 bigscreen:p-12 bg-white rounded-tr-2xl rounded-bl-2xl md:rounded-bl-0 rounded-br-2xl flex flex-col">
        ...
        <UserMintInfoContext.Provider value={mintInfo}>
          { mintInfo.claimedStream ? <MintedAndClaimedContent /> : <ClaimStreamContent /> }
