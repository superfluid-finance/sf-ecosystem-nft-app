...
export const LoggedInWallet = () => {
  ...
  const performSwitchChain = useCallback(async (wallet: any) => {
  ...
  return (
    <div className="flex gap-x-4 items-center cursor-pointer relative" onClick={() => setOpenDropdown(!openDropdown)}>
      <div className="blockie-wrapper" ref={blockieRefDiv}></div>
      <div className="flex flex-col">
        <span className="text-black font-medium">{truncateAddress(user?.wallet?.address as string)}</span>
        {/** @ts-ignore */}
        <span className="text-darkgray -mt-1 text-[12px]">{Number(userBalance).toFixed(4)} {viemWalletClient?.chain?.nativeCurrency?.symbol}</span>
      </div>

      <div className={`rotate-180 transition-all ${openDropdown ? 'rotate-0' : ''}`}>
        <DropdownArrow />
      </div>

      {/** Display disconnect button */}
      { openDropdown && 
      <div className="cursor-pointed flex gap-x-1 items-center absolute bottom-[-130%] right-0 rounded-[0.625rem] py-2.5 px-4 bg-white"
        onClick={() => logout()}>
