...
export const useCheckMintStatus = (triggerUpdate?: boolean) => {
   ...
   **/
  const checkMintedChain = useCallback(async (wallet: any) => {
      ...
      for(let i = 0; i < NETWORK_LIST.length; i++) {
        if (NETWORK_LIST[i].gdaInfo) {
          ...
          const nftContract = getContract({ address: NETWORK_LIST[i].gdaInfo?.nftContractAddress as `0x${string}`, abi: gdaNftContractAbi, client: publicClient })
          const forwarderContract = getContract({ address: NETWORK_LIST[i].gdaInfo?.gdaForwarderV1Address as `0x${string}`, abi: gdaForwarderV1Abi, client: publicClient })

          const result = await nftContract.read.hasMinted([wallet.address])

          if (result) {
            let mintResult: any = await nftContract.read.userMint([wallet.address])
            let hasClaimedStream: any = await forwarderContract.read.isMemberConnected([NETWORK_LIST[i].gdaInfo?.poolAddress!, wallet.address])
  ...
  const checkClaimedStreamStatus = useCallback(async(mintStatus: UserMintInfo) => {
    ...
    const forwarderContract = getContract({ address: mintStatus.mintedChain.gdaInfo?.gdaForwarderV1Address as `0x${string}`, abi: gdaForwarderV1Abi, client: publicClient })
    let hasClaimedStream: any = await forwarderContract.read.isMemberConnected([mintStatus.mintedChain.gdaInfo?.poolAddress!, user?.wallet?.address as `0x${string}`])

    if (hasClaimedStream) {
      let newMintStatus = {...mintStatus, claimedStream: true}
      setUserMintInfo(newMintStatus)
      localStorage.setItem(`${user?.wallet?.address}_sf`, JSON.stringify(newMintStatus))
    } else {
  ...
  useEffect(() => {
    if (authenticated && user?.wallet?.address) {
      ...
      if (wallet && window.localStorage.getItem(`${user?.wallet?.address}_sf`)) {
        // @ts-ignore
        let mintStatus = JSON.parse(window.localStorage.getItem(`${user?.wallet?.address}_sf`))
        ...
        if (!mintStatus.claimedStream) {
          ...
          setUserMintInfo(JSON.parse(window.localStorage.getItem(`${user?.wallet?.address}_sf`)!))
        }
      }
      else if (wallet) {
