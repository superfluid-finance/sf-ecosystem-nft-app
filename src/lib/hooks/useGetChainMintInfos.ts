...
export const useGetChainMintInfos = () => {
  ...
  const getChainMintInfos = async () => {
    ...
    for (let i = 0; i < NETWORK_LIST.length; i++) {
      let lastMintDetails: ChainMintInfo = {
        mintedCount: 0,
        lastMintedTimestamp: undefined,
        flowRunsUntil: undefined,
      };
      ...
      if (NETWORK_LIST[i].gdaInfo) {
        ...
        if (lastMintTimestamp > 0) {
          ...
          lastMintDetails.flowRunsUntil = Number(
            new Date(lastMintDetails.lastMintedTimestamp + FLOW_DURATION),
          );
        }
      }
